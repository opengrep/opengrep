...
CLASS = {
    "sqli": dict(
        ...
        not_neutralizers=["HTML or URL encoding", "URL decoding", "case changes", "trimming"],
    ...
    "cmdi": dict(
        ...
        not_neutralizers=["HTML or URL encoding that keeps quotes, semicolons or spaces", "URL decoding"],
        safe_sink=["the value is one element of an argument array or list and no element is a shell such as "
                   "sh -c or cmd.exe /c that re-parses a string containing the value"],
    ...
    "xss": dict(
        ...
        not_neutralizers=["URL decoding", "Base64 decoding", "SQL escaping", "case changes"],
    ...
    "pathtraver": dict(
        ...
        not_neutralizers=["HTML encoding", "URL decoding", "prefixing a base directory"],
        safe_sink=["the file path is a constant or a validated file name"],
        unsafe_sink=["the value forms part of the path given to a file API"]),
    "ldapi": dict(
        name="LDAP injection",
        neutralizers=["ESAPI encodeForLDAP or encodeForDN", "allow-list validation"],
        not_neutralizers=["HTML or URL encoding", "URL decoding"],
    ...
    "trustbound": dict(
        ...
        not_neutralizers=["encoding of any kind"],
...
def questions_for(cat):
    ...
    if cat in CLASS:
        q.update({
            ...
            "sink_safe_by_construction": Noul(
