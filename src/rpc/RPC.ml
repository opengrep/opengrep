      ...
      | None ->
          Error
            "Dependency resolution is a proprietary feature, but semgrep-pro \
             has not been loaded")
  (* TODO: remove CallDumpRulePartitions from our semgrep_interfaces fork
   * (with the rest of the dump_rule_partitions types), then this arm *)
  | `CallDumpRulePartitions _params ->
      Error "Dump rule partitions is not supported"
