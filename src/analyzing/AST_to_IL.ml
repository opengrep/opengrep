...
let add_stmts env xs = { env with stmts = xs @ env.stmts } 
...
and pattern_assign_statements env ?(eorig = NoOrig) exp pat : env * stmt list =
  ...
  with
  | Fixme (kind, any_generic) -> (env, fixme_stmt kind any_generic)
...
and assign env ~g_expr lhs tok rhs_exp =
  ...
  | G.DeRef _ -> (
      ...
      | Fixme (kind, any_generic) ->
          ...
          let env = add_instr env (mk_i (Assign (fixme_lval, rhs_exp)) eorig) in
...
and expr_aux env ?(void = false) g_expr =
  ...
  | G.Call
      ...
      let env, res =
        ...
        | Lang.Ruby -> (
            ...
            | Fixme _ -> (env, fresh_lval ~str:"Fixme" env tok))
  ...
  | G.LetPattern (pat, e) ->
      ...
      let env, stmts = pattern_assign_statements env ~eorig exp pat in
      let env = add_stmts env stmts in
  ...
  | G.Conditional (e1_gen, e2_gen, e3_gen) ->
      ...
      let env1, e1 = expr { env with stmts = [] } e1_gen in
      let ss_for_e1 = env1.stmts in
      let env2, e2 = expr { env with stmts = [] } e2_gen in
      let ss_for_e2 = env2.stmts in
      let env3, e3 = expr { env with stmts = [] } e3_gen in
      let ss_for_e3 = env3.stmts in
...
and expr env ?void e_gen : env * exp =
  ...
  | Fixme (kind, any_generic) ->
      (env, fixme_exp kind any_generic (related_exp e_gen))
...
and stmt_expr env ?g_expr st =
  ...
  | G.Block (_, block, _) -> (
      ...
      | st :: rev_sts ->
          let env, stmts = List.fold_left_map stmt env (List.rev rev_sts) in
          let env = add_stmts env (List.concat stmts) in
          stmt_expr env st
      | __else__ -> todo ())
  | G.Return (t, eorig, _) ->
...
and lval_of_ent env ent =
  match ent.G.name with
  | G.EN (G.Id (id, idinfo)) -> (env, lval_of_id_info env id idinfo)
...
and stmt_aux env st =
  ...
  | G.Continue (tok, lbl_ident, _) -> (
      ...
      | G.LId lbl -> (env, [ mk_s (Goto (tok, label_of_label env lbl)) ])
      | G.LInt _
      | G.LDynamic _ ->
          todo (G.S st))
  ...
  | G.OtherStmtWithStmt (G.OSWS_Block _, [ G.E objorig ], stmt1) ->
      ...
      let env, stmts = stmt env stmt1 in
      (env, ss @ stmts)
  (* Rust: unsafe block *)
...
and cases_to_exp env tok cases =
  ...
  let env, ss, es =
    List.fold_left
      ...
      (env, [], []) cases
  in
  (env, ss, { e = Operator ((Or, tok), mk_unnamed_args es); eorig = related_tok tok })
...
and cases_and_bodies_to_stmts env tok break_label translate_cases = function
  ...
  | [ G.CasesAndBody ([ G.Default dtok ], body) ] ->
      ...
      let env, stmts = stmt env body in
      (env, [ mk_s (Goto (dtok, label)) ], mk_s (Label label) :: stmts)
  | G.CasesAndBody (cases, body) :: xs ->
      let env, jumps, bodies =
        cases_and_bodies_to_stmts env tok break_label translate_cases xs  (* TODO this is not tail recursive *)
      in
      let label = fresh_label ~label:"__switch_case" env tok in
      let env, case_ss, case = translate_cases cases in
      let jump =
        mk_s (IL.If (tok, case, [ mk_s (Goto (tok, label)) ], jumps))
      in
      let env, stmts = stmt env body in
      let body = mk_s (Label label) :: stmts  in
...
and python_with_stmt env manager opt_pat body =
  ...
  in
let env, stmts = stmt env body in
  env, ss_def_mgr @ ss_def_pat @ stmts
(*****************************************************************************)
(* Defs *)
(*****************************************************************************)
...
let function_definition lang ?ctx fdef =
  ...
  let _, fd = function_definition env fdef in fd

let stmt lang st =
  let env = empty_env lang in
  let _, stmts =  stmt env st in stmts

let expr lang e =
  let env = empty_env lang in
   let _, e = expr env e in e
