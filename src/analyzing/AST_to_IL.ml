...
let mk_aux_var ?(force = false) ?str (env, stmts) tok exp =
  ...
  | __else__ ->
      ...
      let (env, stmts) = add_instr (env, stmts) (mk_i (Assign (lval, exp)) NoOrig) in
      ((env, stmts), var, lval)
...
let add_call (env, stmts) tok eorig ~void mk_call =
  if void then
    ...
    let (env, stmts) = add_instr (env, stmts) (mk_i (mk_call (Some lval)) eorig) in
    ((env, stmts), mk_e (Fetch lval) NoOrig)

let add_stmt (env, stmts) st = (env,  st :: stmts)
let add_stmts (env, stmts) xs =  (env, xs @ stmts)
...
let rec lval (env, stmts) eorig =
  ...
  | G.IdSpecial (G.This, tok) -> ((env, stmts), lval_of_base (VarSpecial (This, tok)))
  | G.DotAccess (e1orig, tok, field) ->
      let (env, stmts), offset' =
        match field with
        | G.FN (G.Id (id, idinfo)) -> ((env, stmts), Dot (var_of_id_info id idinfo))
        | G.FN name ->
...
and pattern (env, stmts) pat =
  ...
  | G.PatTuple (_tok1, pats, tok2) ->
      ...
      in
      let flatten_ss = List.concat ss in
...
and pattern_assign_statements (env, stmts) ?(eorig = NoOrig) exp pat : (env * stmts) * stmt list =
  try
    let (env, stmts), lval, ss = pattern (env, stmts) pat in
    ((env, stmts), [ mk_s (Instr (mk_i (Assign (lval, exp)) eorig)) ] @ ss)
  with
  | Fixme (kind, any_generic) -> ((env, stmts), fixme_stmt kind any_generic)
...
and assign (env, stmts) ~g_expr lhs tok rhs_exp =
  ...
  | G.DeRef _ -> (
      try
        ...
        let (env, stmts) = add_instr (env, stmts) (mk_i (Assign (lval, rhs_exp)) eorig) in
      ...
      | Fixme (kind, any_generic) ->
          ...
          let (env, stmts) = add_instr (env, stmts) (mk_i (Assign (fixme_lval, rhs_exp)) eorig) in
  ...
  | G.Container (((G.Tuple | G.List | G.Array) as ckind), (tok1, lhss, tok2)) ->
      ...
      let tmp = fresh_var (env, stmts) tok2 in
      let tmp_lval = lval_of_base (Var tmp) in
      let (env, stmts) = add_instr (env, stmts) (mk_i (Assign (tmp_lval, rhs_exp)) eorig) in
      (* Ei = tmp[i] *)
  ...
  | G.Record (tok1, fields, tok2) ->
      assign_to_record (env, stmts) (tok1, fields, tok2) rhs_exp (related_exp lhs)
  | _ ->
      let (env, stmts) =
        add_instr (env, stmts) (fixme_instr ToDo (G.E g_expr) (related_exp g_expr))
      in
...
and assign_to_record (env, stmts) (tok1, fields, tok2) rhs_exp lhs_orig =
  ...
  and do_field (env, stmts) acc_rev_offsets f =
    ...
    | G.F
        ...
        in
        let (env, stmts) =
          add_instr (env, stmts) (mk_i (Assign (vari_lval, ei)) (related_tok tok))
        in
    ...
    | G.F
        ...
        let fldi = var_of_id_info id1 ii1 in
        let offset = { o = Dot fldi; oorig = NoOrig } in
        let (env, stmts), fields = do_fields (env, stmts) (offset :: acc_rev_offsets) fields in
        ((env, stmts), Field (fldi, mk_e (RecordOrDict fields) (related_tok tok)))
    | field ->
        ...
        let (env, stmts) =
          add_instr (env, stmts) (mk_i (Assign (tmpi_lval, ei)) (related_tok tok1))
        in
        ((env, stmts), Field (xn, mk_e (Fetch tmpi_lval) (Related (G.Fld field))))
  in
...
and expr_aux (env, stmts) ?(void = false) g_expr =
  ...
  | G.Call ({ e = G.IdSpecial (G.Op op, tok); _ }, args) -> (
      ...
      if not void then ((env, stmts), mk_e (Operator ((op, tok), args)) eorig)
        ...
        | obj :: args' ->
            ...
            in
            let method_name = fresh_var (env, stmts) tok ~str:(Tok.content_of_tok tok) in
            ...
            add_call (env, stmts) tok eorig ~void (fun res -> Call (res, method_, args'))
      )
  | G.Call
      ...
      | [ G.Arg e ] ->
          ...
          let opexp =
            mk_e
              ...
              (related_tok tok)
          in
          let (env, stmts) = add_instr (env, stmts) (mk_i (Assign (lval, opexp)) eorig) in
          ((env, stmts), lvalexp)
  ...
  | G.Call
      ...
      let (env, stmts), args = arguments (env, stmts) (Tok.unbracket args) in
      let (env, stmts) =
        add_instr (env, stmts) (mk_i (CallSpecial (Some lval, special, args)) eorig)
      in
  ...
  | G.New (tok, ty, _cons_id_info, args) ->
      ...
      let (env, stmts) = add_instr (env, stmts) (mk_i (New (lval, t, None, args)) eorig) in
      ((env, stmts), mk_e (Fetch lval) NoOrig)
  | G.Call ({ e = G.IdSpecial spec; _ }, args) -> (
      ...
      | Fixme (kind, any_generic) ->
          ...
          add_call (env, stmts) tok eorig ~void (fun res -> Call (res, fixme, args)))
  | G.Call (e, args) ->
      ...
      let ident_function_call_hack exp =
        ...
        | { base = Var { ident; id_info; _ }; _ }
          when env.lang =*= Lang.Ruby
            ...
            let tok = G.fake "call" in
            add_call (env, stmts) tok eorig ~void (fun res -> Call (res, exp, []))
        | _ -> ((env, stmts), exp)
      in
  ...
  | G.LetPattern (pat, e) ->
      ...
      let (env, stmts), new_stmts = pattern_assign_statements (env, stmts) ~eorig exp pat in
  ...
  | G.Seq xs -> (
      ...
      | last :: xs ->
          ...
          xs
          |> List.iter (fun e ->
                 let _eIGNORE = expr (env, stmts) e in
                 ());
          expr (env, stmts) last)
  ...
  | G.Container (kind, xs) ->
      let l, xs, r = xs in
      let (env, stmts), xs = List.fold_left_map (fun (env, stmts) x -> expr (env, stmts) x) (env, stmts) xs in
  ...
  | G.Lambda fdef ->
      ...
      in
      let (env, stmts) = add_instr (env, stmts) (mk_i (AssignAnon (lval, Lambda fdef)) eorig) in
      ((env, stmts), mk_e (Fetch lval) eorig)
  | G.AnonClass def ->
      (* TODO: should use def.ckind *)
      let tok = Common2.fst3 def.G.cbody in
      let lval = fresh_lval (env, stmts) tok in
      let (env, stmts) = add_instr (env, stmts) (mk_i (AssignAnon (lval, AnonClass def)) eorig) in
      ((env, stmts), mk_e (Fetch lval) eorig)
  ...
  | G.Constructor (cname, (tok1, esorig, tok2)) ->
      ...
      let (env, stmts), es =
        List.fold_left_map (fun (env, stmts) eiorig -> expr (env, stmts) eiorig) (env, stmts) esorig
      in
      ((env, stmts), mk_e (Composite (Constructor cname, (tok1, es, tok2))) eorig)
  ...
  let ((env, stmts), _), args' =
    ...
    |> List.fold_left_map
         (fun ((env, stmts), cond) argi ->
           let (env, stmts), ssi, argi' = arg_with_pre_stmts (env, stmts) argi in
           let (env, stmts) =
             if ssi <> [] then add_stmt (env, stmts) (mk_s @@ If (tok, cond, ssi, []))
             else (env, stmts)
           in
           let condi =
...
and call_special (_env, _stmts) (x, tok) =
  ( (match x with
    | G.Op _
    | G.IncrDecr _
    | G.This
    ...
    List.fold_left_map
      (fun (env, stmts) x ->
        ...
        | G.F
            ...
            let (env, stmts), field_def =
              ...
              | G.FuncDef fdef ->
                  ...
                  let (env, stmts) =
                    add_instr (env, stmts) (mk_i (AssignAnon (lval, Lambda fdef)) forig)
                  in
                  ((env, stmts), mk_e (Fetch lval) forig)
              | ___else___ -> ((env, stmts), todo (G.E e_gen))
            in
        ...
        | _ when is_hcl env.lang ->
            ...
            ((env, stmts), None)
        | G.F _ -> ((env, stmts), todo (G.E e_gen)))
    ...
    List.fold_left_map
      (fun (env, stmts) x ->
        ...
        | G.XmlXml xml' ->
            ...
            let (env, stmts), xml = xml_expr (env, stmts) ~void:false eorig' xml' in
            ((env, stmts), Some xml)
        | G.XmlExpr (_, None, _)
        | G.XmlText _ ->
            ((env, stmts), None))
...
and stmt_expr (env, stmts) ?g_expr st =
  ...
  | G.If (tok, cond, st1, opt_st2) ->
      ...
      let (env, stmts), pre_a1, e1 = stmt_expr_with_pre_stmts (env, stmts) st1 in
  ...
  | G.Block (_, block, _) -> (
      ...
      | st :: rev_sts ->
          let (env, stmts), new_stmts = List.fold_left_map stmt (env, stmts) (List.rev rev_sts) in
  ...
  | G.DefStmt (ent, G.VarDef { G.vinit = Some e; vtype = opt_ty; vtok = _ })
    when def_expr_evaluates_to_value env.lang ->
      ...
      let (env, stmts) = mk_i (Assign (lv, e)) (Related (G.S st)) |> add_instr (env, stmts) in
...
and lval_of_ent (env, stmts) ent =
  match ent.G.name with
  | G.EN (G.Id (id, idinfo)) -> ((env, stmts), lval_of_id_info (env, stmts) id idinfo)
  | G.EN name -> lval (env, stmts) (G.N name |> G.e)
  | G.EDynamic eorig -> lval (env, stmts) eorig
  | G.EPattern (PatId (id, id_info)) -> lval (env, stmts) (G.N (Id (id, id_info)) |> G.e)
  | G.EPattern _ -> (
      let any = G.En ent in
      log_fixme ToDo any;
...
and cond_with_pre_stmts (env, stmts) cond =
  with_pre_stmts (env, stmts) (fun (env, stmts) ->
      ...
      | G.OtherCond
          ...
          let (env, stmts), e' = expr (env, stmts) e in
          let (env, stmts), lv = lval_of_ent (env, stmts) ent in
          let (env, stmts) = add_instr (env, stmts) (mk_i (Assign (lv, e')) (Related def)) in
  ...
  let (env, stmts), list_of_lists =
    List.fold_left_map
      (fun (env, stmts) x ->
        match x with
        | G.ForInitExpr e ->
            let (env, stmts), ss, _eIGNORE = expr_with_pre_stmts (env, stmts) e in
            ((env, stmts), ss)
      ...
      (env, stmts) xs
  in
  ((env, stmts), List.concat list_of_lists (*TODO this is not tail recursive!!!!*))
...
and type_ (env, stmts) (ty : G.type_) : (env * stmts) * G.type_ =
  ...
  let (env, stmts), _ =
    List.fold_left_map
      (fun (env, stmts) e ->
        let (env, stmts), x, y = mk_aux_var ~force:true ~str:"_type" (env, stmts) tok e in
        ((env, stmts), (x, y)))
      (env, stmts) exps
  in
  ((env, stmts), ty)

and type_with_pre_stmts (env, stmts) ty = with_pre_stmts (env, stmts) (fun (env, stmts) -> type_ (env, stmts) ty)

and type_opt (env, stmts) opt_ty : (env * stmts) * unit =
  Option.fold ~none:((env, stmts), ())
    ~some:(fun ty ->
      let (env, stmts), _ = type_ (env, stmts) ty in
      ((env, stmts), ()))
    opt_ty
...
and type_opt_with_pre_stmts (env, stmts) opt_ty =
  let (env, stmts), ss, () = with_pre_stmts (env, stmts) (fun (env, stmts) -> type_opt (env, stmts) opt_ty) in
  ((env, stmts), ss)
...
and implicit_return (env, stmts) eorig tok =
   ...
   *)
  let (env, stmts), ss, e = expr_with_pre_stmts ~void:false (env, stmts) eorig in
...
and mk_class_construction (env, stmts) obj origin_exp ty cons_id_info args :
  ...
  let (env, stmts), ss1, args' = args_with_pre_stmts (env, stmts) (Tok.unbracket args) in
  let opt_cons =
...
and stmt_aux (env, stmts) st =
  ...
  | G.ExprStmt (eorig, tok) -> (
      match eorig with
      | { is_implicit_return = true; _ } -> implicit_return (env, stmts) eorig tok
  ...
  | G.DirectiveStmt dir -> ((env, stmts), [ mk_s (MiscStmt (DirectiveStmt dir)) ])
  | G.Block xs ->
      let xs = xs |> Tok.unbracket in
      let (env, stmts), list_of_lists = List.fold_left_map stmt (env, stmts) xs in
  ...
  | G.Switch (tok, switch_expr_opt, cases_and_bodies) ->
      let (env, stmts), ss, translate_cases =
        ...
        | Some switch_expr ->
            let (env, stmts), ss, switch_expr' = cond_with_pre_stmts (env, stmts) switch_expr in
            ( (env, stmts),
              ss,
      ...
      let (env, stmts), jumps, bodies =
        cases_and_bodies_to_stmts (switch_env, switch_stmts) tok break_label translate_cases
          cases_and_bodies
      in
  ...
  | G.For (tok, G.MultiForEach (FE fr :: for_eachs), st) ->
      for_each (env, stmts) tok fr (G.For (tok, G.MultiForEach for_eachs, st) |> G.s)
  ...
  | G.Continue (tok, lbl_ident, _) -> (
      ...
      | G.LId lbl -> ((env, stmts), [ mk_s (Goto (tok, label_of_label (env, stmts) lbl)) ])
      | G.LInt _
      | G.LDynamic _ ->
          todo (G.S st))
  ...
  | G.Assert (tok, args, _) ->
      let (env, stmts), ss, args = args_with_pre_stmts (env, stmts) (Tok.unbracket args) in
  ...
  | G.Try (_tok, try_st, catches, opt_else, opt_finally) ->
      try_catch_else_finally (env, stmts) ~try_st ~catches ~opt_else ~opt_finally
  ...
  | G.OtherStmtWithStmt (G.OSWS_Block _, [ G.E objorig ], stmt1) ->
      ...
      let (env, stmts), ss, _TODO_obj = expr_with_pre_stmts (env, stmts) objorig in
...
and for_each (env, stmts) tok (pat, tok2, e) st =
  let cont_label_s, break_label_s, (st_env, st_stmts) = mk_break_continue_labels (env, stmts) tok in
  let (env, stmts), ss, e' = expr_with_pre_stmts (env, stmts) e in
  let _, st = stmt (st_env, st_stmts) st in
...
and switch_expr_and_cases_to_exp (env, stmts) tok switch_expr_orig switch_expr cases =
...
and cases_and_bodies_to_stmts (env, stmts) tok break_label translate_cases = function
  ...
  | [ G.CasesAndBody ([ G.Default dtok ], body) ] ->
      ...
      ((env, stmts), [ mk_s (Goto (dtok, label)) ], mk_s (Label label) :: new_stmts)
...
and python_with_stmt (env, stmts) manager opt_pat body =
  ...
  let (env, stmts), ss_def_mgr =
    let (env, stmts), ss_mk_mgr, manager' = expr_with_pre_stmts (env, stmts) manager in
    ((env, stmts), ss_mk_mgr @ [ mk_s (Instr (mk_i (Assign (mgr, manager')) NoOrig)) ])
  in
  (* PAT = mgr *)
  let (env, stmts), ss_def_pat =
    ...
    | None -> ((env, stmts), [])
    | Some pat ->
        pattern_assign_statements (env, stmts) (mk_e (Fetch mgr) NoOrig) ~eorig:NoOrig
          pat
  in
  let (env, stmts), new_stmts = stmt (env, stmts) body in
...
and function_definition (env, stmts) fdef =
  ...
  ((env, stmts), { fkind = fdef.fkind; fparams; frettype = fdef.G.frettype; fbody })
