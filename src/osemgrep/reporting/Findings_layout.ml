...
let pp_trace_location ?(finding_path : Fpath.t option) ~(prefix : string)
...
let pp_dataflow_tree ?(finding_path : Fpath.t option)
  ...
  |> List.iteri (fun (i : int) (step : trace_step) ->
         ...
         step.locations
         |> List.iter
              (pp_trace_location ?finding_path ~prefix ~gutter ~gutter_blank
                 ~highlight ppf))
...
let pp_dataflow_trace ?(finding_path : Fpath.t option)
  ...
  |> List.iter (fun (step : trace_step) ->
         ...
         step.locations
         |> List.iter
              (pp_trace_location ?finding_path ~prefix:indent ~gutter
