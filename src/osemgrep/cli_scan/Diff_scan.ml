              ...
              let baseline_targets, baseline_diff_targets =
                if interfile_in_play then
                  (* An interfile match depends on files that carry no match of
                     ...
                     gets reported as newly introduced.  [paths_in_scanned] is
                     no help either, since a diff scan already narrowed the head
                     to the changed files, so take the baseline's own full
                     target set. *)
