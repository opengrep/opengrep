...
let engine_type_conf ~oss ~taint_intrafile ~taint_interfile ~pro ~secrets
  ...
  then
    Error.abort
      "Mutually exclusive options --oss/--taint-intrafile/--taint-interfile/--pro";
  ...
  if oss then Engine_type.OSS
    ...
    let analysis =
      Engine_type.(
        ...
        | _ when taint_interfile -> Interprocedural
    ...
    | false, Interprocedural, None when taint_intrafile || taint_interfile -> OSS  (* Allow taint_intrafile/taint_interfile in OSS *)
    | _ ->
        PRO
          {
            extra_languages;
