...
let regexp_matcher ?(base_offset = 0) (big_str : string) (file : Fpath.t)
    (regexp : Pcre2_.t) =
  ...
  |> List_.map (fun sub ->
         ...
         *)
         ...
         let names =
           Pcre2.names (Pcre2_.pcre_regexp regexp) |> Array.to_list
         in
                      ...
                      | Not_found ->
                          Log.debug (fun m ->
                              m "not found %d substring of %s in %s" n
                                (Pcre2_.pcre_pattern regexp) matched_str);
                          None)
         in
         let names_env =
           names
                  ...
                  try
                    ...
                    let bytepos, _ =
                      Pcre2.get_named_substring_ofs
                        (Pcre2_.pcre_regexp regexp)
                        name sub
                    in
                    let bytepos = bytepos + base_offset in
                    let str =
                      Pcre2.get_named_substring
                        (Pcre2_.pcre_regexp regexp)
                        name sub
                    in
                    let line, column = line_col_of_charpos file bytepos in
