...
static bool inner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
    ...
    // NONE == ts_external_token_semgrep_ellipsis (index 68, appended last in externals).
    // Emit semgrep_ellipsis for a standalone "..." that is not immediately followed by
    // expression-starting characters (would make it a beginless range like ...expr)
    // or by ">" (would make it the "...>" closing token of a deep ellipsis).
    if (valid_symbols[NONE] && !valid_symbols[BEGINLESS_RANGE_OPERATOR] && lexer->lookahead == '.') {
        lexer->advance(lexer, false);
        ...
        bool starts_expr_or_deep_close =
                           (after >= '0' && after <= '9') ||
                           (after >= 'a' && after <= 'z') ||
                           (after >= 'A' && after <= 'Z') ||
                           after == '_' || after == '(' || after == '"' ||
