...
if [[ "$(uname -s)" =~ ^(CYGWIN|MINGW|MSYS) ]]; then
  EXTRA_ARGS+=(
    --include-data-files=src/semgrep/bin/*.exe=semgrep/bin/
    --include-data-files=src/semgrep/bin/*.dll=semgrep/bin/
  ...
  EXTRA_ARGS+=(
    --include-data-dir=src/semgrep/bin=semgrep/bin
  )
fi
...
"$PYTHON_BIN" -m nuitka \
  ...
  src/semgrep/console_scripts/entrypoint.py
