...
## The second rule syntax

Opengrep also parses an experimental second syntax for rules, in which a rule
matches with `match:` instead of `pattern:` and `patterns:`, and combines
conditions with `all`, `any`, `not`, `inside`, `anywhere`, `as`, `where` and
`focus`. A taint rule written in it uses a `taint:` block with `sources`,
`sinks`, `sanitizers` and `propagators`. This reference does not document that
syntax, because it may still change. The keys above are listed in this page's
`ignore` list, so they do not appear as undocumented.
